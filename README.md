...
Each fork also bounds how much state it keeps in memory and on disk. `scripts/anvil-fork.sh` passes `--prune-history <count>`, `--max-persisted-states 16` and `--transaction-block-keeper <count>` to anvil on both branches, where `<count>` is `3600 / BLOCK_TIME`, one hour of blocks at the fork's own block time (720 blocks at the current `BLOCK_TIME=5`). These replace an earlier `--preserve-historical-states` flag, which kept a full in memory state snapshot per mined block: at the one to two second block times each fork used before, tens of thousands of blocks accumulated per day and memory grew without bound, and the periodic state dump then had to serialize that ever growing state too, so CPU and disk usage grew along with it. `--state-interval` is raised from 60 to 300 seconds for the same reason, so a fork now dumps its state every five minutes instead of every minute. Three consequences follow directly: a query against a block older than one hour fails, a transaction lookup older than one hour fails, and no more than 16 states persist on disk at once. A hard kill of the container can still lose up to five minutes of state since the last dump; a graceful stop dumps state on exit regardless. An already oversized `/data/state` file from before this change still loads on the next boot, since pruning only bounds growth from that point forward and does not shrink what is already written; deleting the fork's data directory forces a completely fresh, small fork.
