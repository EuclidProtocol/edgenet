...
| `BLOCK_TIME` | Seconds between blocks, passed to anvil as `--block-time`. Set literally in `docker-compose.yml`, not in `.env`, at 5 seconds on all three forks (`anvil-base`, `anvil-somnia` and `anvil-polygon` previously used 2, 1 and 2, each closer to that chain's own real block time). `BLOCK_TIME` also sizes the state pruning window described in [How each anvil fork boots](#how-each-anvil-fork-boots): `scripts/anvil-fork.sh` keeps `3600 / BLOCK_TIME` blocks of history, one hour's worth, which is 720 blocks at the current value. Required; must be a positive integer. |
...
Both branches also pass `--block-time "$BLOCK_TIME"` and `--chain-id "$EVM_CHAIN_ID"`. Both are required: a missing, non-positive-integer, or quote-containing value is fatal before anvil starts, same as a malformed `FORK_BLOCK`. Without `--block-time`, anvil auto-mines a block on every transaction and produces no empty blocks, so an idle fork never advances; with it, the fork ticks on a fixed interval like a real chain. `--chain-id` is not a no-op: anvil does not have to inherit the chain id from the chain it forks, and the flag overrides whatever it would otherwise report. A fork of Base with no `--chain-id` flag reports 8453 over `eth_chainId`; passed as `--chain-id 84539`, it reports 84539 instead. That override is the reason `EVM_CHAIN_ID` is passed at all: each fork's real mainnet chain id (Base 8453, Somnia 5031, Polygon 137) is offset by one inserted digit (84539, 50319, 1379) rather than reported as is, so a wallet or browser extension already configured for the real chain cannot mistake the fork for it. Both values are hardcoded per service in `docker-compose.yml` (`anvil-base`, `anvil-somnia` and `anvil-polygon` all use 5 seconds for block time, and chain ids 84539, 50319 and 1379 respectively), not sourced from `.env`. The chain id is a fact about the upstream chain, queried live from each RPC endpoint via `eth_chainId` before being offset; the block time is no longer each chain's own real block time (`anvil-base` and `anvil-polygon` previously used 2 seconds, `anvil-somnia` used 1), it is now the same 5 seconds on all three, chosen together with the one hour state pruning window described below. Deliberately not overridden: anvil already inherits gas limit, base fee, hardfork and code size limit from the forked chain, and its own defaults for transaction ordering and epoch length are already realistic, so none of those were touched. One caveat: anvil's block time only accepts whole seconds, so `anvil-somnia`, whose real block time is well under a second, still ticks slower than the real chain; see [DEVELOPER.md](DEVELOPER.md#8-known-gaps-and-cleanup-backlog) for this and other known gaps.

Each fork also bounds how much state it keeps on disk and how many old transactions it retains. `scripts/anvil-fork.sh` passes `--max-persisted-states 16` and `--transaction-block-keeper <count>` to anvil on both branches, where `<count>` is `3600 / BLOCK_TIME`, one hour of blocks at the fork's own block time (720 blocks at the current `BLOCK_TIME=5`). `--prune-history` is deliberately not passed: anvil rejects it when combined with `--max-persisted-states`, and `--prune-history` also disables state persistence outright, which these forks depend on to survive restarts via `/data/state`. These flags replace an earlier `--preserve-historical-states` flag, which kept a full in memory state snapshot per mined block: at the one to two second block times each fork used before, tens of thousands of blocks accumulated per day and memory grew without bound, and the periodic state dump then had to serialize that ever growing state too, so CPU and disk usage grew along with it. `--state-interval` is raised from 60 to 300 seconds for the same reason, so a fork now dumps its state every five minutes instead of every minute. Two consequences follow directly: a transaction lookup older than one hour fails, and no more than 16 states persist on disk at once. A hard kill of the container can still lose up to five minutes of state since the last dump; a graceful stop dumps state on exit regardless. An already oversized `/data/state` file from before this change still loads on the next boot, since these bounds only limit growth from that point forward and do not shrink what is already written; deleting the fork's data directory forces a completely fresh, small fork.
...
### Keeping fork clocks in sync

Anvil resumes a persisted state's own clock after a restart (see [How each anvil fork boots](#how-each-anvil-fork-boots) above for why state persists at all). A fork that sat stopped for a while therefore comes back reporting a chain time that lags the real, wall clock time by roughly however long it was down.

The faucet page has a `Sync fork clocks` button for this. Clicking it sends `POST /api/sync-time`, which checks `base`, `somnia` and `polygon` and reports, per chain, whether it was already in sync or how far behind it was before being corrected.

The server also checks the same three forks on a timer of its own, independent of the button. `CLOCK_SYNC_INTERVAL_SECONDS`, an environment variable defaulting to 600 (ten minutes), sets that timer's period in seconds. Set it to 0 to disable the timer and rely only on the button, or on a direct call to `/api/sync-time`.

Either path only corrects a fork whose clock lags the wall clock by more than 30 seconds; a fork within that threshold is left alone and reported as already in sync. The threshold exists because `evm_setNextBlockTimestamp` only accepts a timestamp strictly ahead of the fork's current tip, so a clock that has barely drifted has little room to be corrected upward at all. When a fork is behind by more than the threshold, the sync sets the next block's timestamp to the current wall clock time and mines it; block time mining then derives every later block's timestamp from that corrected one, so a single corrective block per sync is enough.
